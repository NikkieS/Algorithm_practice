...
Date : 10/10/20
Author : GaRam Song
Description : datetime algorithm
문제 설명
...
if 'PM' in p :
    temp = datetime.datetime.strptime(p[3:], '%H:%M:%S')
...
for i in range(3):
    time[i] = int(time[i])  # str -> int

print(time)


if p[0:2] == 'PM' and time[0] != 12 :
    time[0] += 12
    
elif p[0:2] == 'AM' and time[0] == 12 :
    time[0] = 0
